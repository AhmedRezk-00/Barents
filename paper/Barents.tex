...
\begin{abstract}
    Data is becoming more and more important. Which is why more and more people have to access that data for their own uses. To enable all kinds of domain experts to do that, the Universität Stuttgart has developed BARENTS, a "tailorable data preparation zone for data lakes" \cite{barents21}. We compared several methods of implementation and developed a prototype.
\end{abstract}
...
In our modern society, data is more important than it has been ever before. Whether for day to day applications like navigation or social media, chatbots like ChatGPT, that are recently open to the public, or even in different study fields like molecular biotechnology using genomics data, many need some form of data. \newline
To allow all sort of domain experts to access that data without needing a deep understanding of databases, we want to create a prototype, an editor, for the concept BARENTS. BARENTS "enables users to model in an ontology how to derive information from data and assign the information to use cases" \cite{barents21}. The users are not required to understand the background process, only the basic steps. \newline
The Barents paper is organized as follows: Section \ref{Section2} discusses the background of Barents and its overall concept. Section \ref{Section3} discusses our requirement specification for building the prototype BARENTS. Section \ref{Section4} evaluates ontology editors and whether they can be used for the BARENTS prototype. Section \ref{Section5} outlines the prototype implementation and the structure of the frontend and backend system. In Section \ref{Section6} we evaluate our requirement specification from Section \ref{Section3}. Finally, in Section 7 we conclude and summarize our work.

\section{Background}\label{Section2}
\label{background}

    We want to develop our Editor with the concept BARENTS. In this section we will have a closer look at BARENTS and introduce a scenario, in which our Editor might be of use: testing chocolate samples for nut allergens. This section \ref{background} including its pictures is mainly based on papers by Stach et al.\cite{barents21} and \cite{barents22}.
    ...
    \subsection{Overview BARENTS}
        ...
        \begin{figure}[h]
            ...
            \caption{operation process of BARENTS \cite{barents21}}
    ...
    \subsection{Use case scenario: nut allergens in chocolate samples}
        ...
        \begin{figure} [H]
            ...
            \caption{graphical depiction of the BARENTS ontology for the chocolate example \cite{barents21}}
        ...
        \begin{figure} [H]
            ...
            \caption{from the chocolate example created RDF/XML file \cite{barents21}}
            \label{RDFExample}
        \end{figure}



\section{Requirement Specification}\label{Section3}
    \begin{enumerate}
        \item[(R1)] Our goal is an Easy-to-use GUI for domain experts, with which they can create ontologies.
        \item[(R2)]  minimum one of the Transformations I,II or III as defined in \ref{barents} (namely filter, map or reduce operator) is functioning.
...
\section{Implementation}\label{Section4}
    \subsection{Discussion on own implementation vs. existing tools}
        ...
        \subsubsection{Protégé}
            Protégé is an open Source ontology editor\cite{musen2015protege}. One of its main core features that you can extend its functionality by building your own plugins. However after carefully examining and using Protégé, we determine that the UI is complex, as it requires us to watch multiple tutorials just to navigate and use it effectively, which goes against our easy-to-use requirement (R1).
            Moreover, the process of developing a Protégé plugin is not clear and the some of the available information became outdated or is no longer accessible. 
        ...
        \subsubsection{implementing ourselves}
            ...
            These mockups,in \autoref{mockups}, were early attempts at implementing a BARENTS editor, each focusing on different challenges.
            The first mockup focused on representing an rdf graph graphically. 
            The focus was on finding an intuitive design language that was both easy to understand and suited for BARENTS.
            The second mockup improved the program architecture and experimented with the pyhon library RDFLib in order to keep the application more structured. 
            Essentially the focus was on managing the rdf graph separately from its representation in the editor. 
            Finally, the third mockup brough these two together and finalized our approach to the BARENTS editor. 
            As our prototype follows closely in these mockups footsteps, we will omit a more detailed discussion of the three mockups. 
             \begin{figure}[h]
                \includegraphics[width = 1\columnwidth]{Mockup.jpg} 
                \Description{three iterative mockups}
                \caption{Three iterative mockups}
                \label{mockups} 
            \end{figure}
    ...
    \subsection{evaluation}
        ...
        \begin{table} [h]
            ...
            \begin{tabulary} {\paperwidth} {p{1,5cm}|p{2,5cm}|p{1,3cm}|p{2,5cm}}
                ...
                OwlGrEd & UML-style classes and connections & Lua & unclear and complex Code, timeconsuming to adapt \\ \hline
                WebVowl & GUI to draw graph&JavaScript& exports only TTL, code structure unclear \\ \hline
                Protégé & extendable via plugins & Java& Complex UI; not enough information to develop a plug in\\ \hline
            ...
            \label{Editor-table}
        \end{table}
        Due to the various problems of modifying existing editors, we finally decided to implement an editor ourselves.


\section{Prototyping}\label{Section5}
    \subsection{Scope of the Prototype}
        ...
        This allows for both the desired proof of concept, while keeping the scale much more feasible.
    ...
    \subsection{Data Access and the Expanded BARENTS Ontology}
        ...
        This information is then modeled as two separate literals.
...
\begin{figure*}[h]
    \centering
    \includegraphics[width = \linewidth]{GUI_interface.PNG} 
    \Description{The prototype of the BARENTS Editor}
    \caption{The prototype of the BARENTS Editor}
    \label{BARENTS-editor} 
\end{figure*}
The User Interface for the Barents project, in \autoref{BARENTS-editor} and in \autoref{FrontendDesign},
...
\section{Evaluation - Requirements Fulfillment}\label{Section6}
    In this section, we evaluate our project towards or previously set requirements in Section \ref{Section3}.
    For the easy-to-use requirement \hyperref[Section3]{R1}, we kept our UI design minimalistic, used the least amount of buttons, and only kept features that are relevant to the creation of an Ontology. These decisions were taken based on the review of other ontology editors that were hard to understand or had unnecessary features for our project, which would overcomplicate things for domain experts.
    Moreover, the \hyperref[Section3]{R3} requirement is fulfilled, as the user can save the created ontologies graphs in various formats, giving flexibility for future Developers to design their own Data Processing engine, that does not only have work RDF/XML files.

    We have built a small data-processing-engine, that can process data according to the ontology, and utilizes SQLite to transform relational databases. The Transformation I, namely filter has been implemented in our engine. However, the filter functionality is limited, as it only works for sqlite Databases. It can be stated that requirements \hyperref[Section3]{R4} (create a data-processing engine), \hyperref[Section3]{R5} (prototype works with at least one relational database) and \hyperref[Section3]{R2} (implementing Transformation I) are fulfilled.

\section{Conclusion}
Our paper presents a proof of concept for the utility of a GUI that allows users to create a configuration file following the BARENTS specifications. \newline
We evaluated other ontology editors and determined that developing our own UI is more efficient, although it would mean a larger workload. Our BARENTS UI can now create an RDF graph with an expanded BARENTS ontology easily for the user and export it. The data processing engine can now process Transformation I (filter). \newline
For future projects, one could extend the processing engine to Transformations II (map) and III (reduce) and improve compatibility with other databases.  
