...
\section{Conclusion}
Our paper presents a proof of concept for the utility of a GUI that allows users to create a configuration file following the BARENTS specifications. We evaluated other ontology editors and determined that developing our own UI is more efficient, although it would mean a larger workload. Our BARENTS UI can now create an RDF graph with an expanded BARENTS ontology easily for the user and export it. The data processing engine can now process Transformation I (filter). However, for future projects, one could extend the processing engine to Transformations II (map) and III (reduce) and improve compatibility with other databases.  
