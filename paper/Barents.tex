...
Data is digital Oil. Within this digital age, it is crucial for humantity in almost every domain. 
Additionally, data is not restricted in quantity, meaning u limited data can be generated for use.
Along with this growing demand, the processing of this data must develop in order to keep up.
The goal has to be a demand-driven data provisioning, i. e., the right data must be available in the right form at the
right time. 
...
he users get the opportunity to decide what data is derived how and where.
The users are not required to understand the background-process, only the basic steps.
We developed a prototype and compared severall methods of implementation.
\end{abstract}
    ...
    \subsection{evaluation}
        ...
        \begin{table} [h]
            ...
            \begin{tabulary} {\paperwidth} {p{1,5cm}|p{2,5cm}|p{1,3cm}|p{2,5cm}}
                ...
                WebVowl & GUI to draw graph&JavaScript,Node.js& exports only TTL, code structure unclear \\ \hline
...
For our backend, we implemented a dynamic filter operation that leverages Python lambda functions, giving the user a wide range of filtering criteria.
